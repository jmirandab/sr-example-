...
const TreeView = ({ onSelectMovie, selectedMovieId: parentSelectedMovieId }) => {
    ...
    return (
        <div className={styles.treeView} role="menu" aria-label={t('movie.selectMovie')}>
                ...
                return (
                    <div key={genre} className={styles.treeGroup}>
                        <div 
                            ...
                            aria-haspopup="true"
                            role="menuitem"
                            aria-label={`${t('movie.genre')} ${genre}`}
                            tabIndex="0"
                        >
                            <span className={`${styles.groupIcon} ${isExpanded ? styles.expanded : ''}`} aria-hidden="true">▶</span>
                        ...
                        <div 
                            ...
                            id={genreId}
                            role="group"
                            aria-label={`${t('movie.movieLabel')} ${genre}`}
                        >
                            {peliculas[genre].map(movie => (
                                <div 
                                    ...
                                    role="menuitem"
                                    aria-label={`${t('movie.movieLabel')} ${movie.title}`}
                                    tabIndex={isExpanded ? "0" : "-1"}
                                    aria-current={selectedMovieId === movie.id ? 'true' : undefined}
                                >
